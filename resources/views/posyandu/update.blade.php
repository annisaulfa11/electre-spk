        ...
        <nav class="flex justify-between items-center">
            <ul>
                <li class="flex items-center">
                    ...
                    <a><span class="text-main font-semibold">Edit Posyandu</span></a>
                </li>
            </ul>
        </nav>
    </div>
    <div class="bg-white border rounded-md mx-3 my-3 py-3 px-4">
        <form action="/posyandu/{{$data->id}}" method="POST" class="flex flex-col gap-y-7">
            @csrf
            @method('put')
            <div class="flex flex-col ">
                <label for="nama_posyandu" class="mb-1 text-main text-xs font-semibold">NAMA POSYANDU</label>
                <input type="text" name="nama_posyandu" id="nama_posyandu" class="h-9 text-gray-500 border rounded-sm border-slate-400 focus:outline-none focus:ring-0 focus:border-main" required value="{{ $data->nama_posyandu}}">
            </div>
            <div class="flex flex-col ">
                <label for="alamat" class="mb-1 text-main text-xs font-semibold">ALAMAT</label>
                <input type="text" name="alamat" id="alamat" class="h-9 text-gray-500 border rounded-sm border-slate-400 focus:outline-none focus:ring-0 focus:border-main" required value="{{ $data->alamat}}">
            </div>
